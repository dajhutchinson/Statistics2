...
\remarkk{Overview of Bayesian Approach}
\begin{enumerate}
...
\remark{Exentision to Remark 1.1 iii)}
...
\proposition{Usefulness of $\beta$-Distribution}
...
\theorem{Generalisation of Prior Beliefs}
...
\proposition{Choosing a Posterior Distribution}
...
\theorem{$\text{Uniform}[0,1]\sim\text{Beta}(1,1)$}
...
\propositionn{Alternative Loss Functions}
\begin{enumerate}
...
\definition{Symmetric $1-\alpha$ Credible Interval}
...
\theorem{The $\mathcal{HPD}(1-\alpha)$ redivle set is the smallest subet of $\Theta$ containing exactly $1-\alpha$ of the total probability.}

\remark{High Posterior Density Sets are harder to determine without a computer than Symmetric Credible Intervals}
...
\remark{Difference to Frequestist Hypothesis Testing}
Consider a model with posterior distribution $p(\theta|\x)$ and assume that we would like to test
$$H_0:\theta\in\Theta_0\text{ against }H_1:\theta\in\Theta_1$$
The difference in \textit{Bayesian Hypothesis Testing}, compared to \textit{Frequentist Hypothesis Testing} is that we can actually calculate
$$\prob(\vartheta\in\Theta_0|\X=\x)=\int_{\Theta_0}p(\theta|\x)d\theta\text{ and }\prob(\vartheta\in\Theta_1|\X=\x)=\int_{\Theta_1}p(\theta|\x)d\theta$$
