...
Let $X_i\overset{\text{iid}}{\sim}\text{Bernoulli}(\theta^*)$ for some $\theta^*\in\Theta=[0,1]$.\\
The MLE and method of moments estimator are the sample mean
$$\hat{\theta}_n(X_1,\dots,X_n)=\frac{1}{n}\sum\limits_{i=1}^nX_i$$
\begin{enumerate}[label=\roman*)]
	\item Consistency of $\{\hat{\theta})n\}$ is provided by the \textit{Weak Law of Large Numbers}, since $\expect(X_1)=\theta^*$.
	\item Now we consider a crude way to \textit{capture} $\theta^*$ in a random interval with high probability. We write $\hat{\theta}_n:=\hat{\theta}_n(X_1,\dots,X_n)$. Note that
	$$\expect(\hat{\theta}_n;\theta^*)=\theta^*\quad\text{and}\quad\var(\hat{\theta}_n;\theta^*)=\frac{\theta^*(1-\theta^*)}{n}$$
	Using \textit{Chebyshev's Inequality}
	$$\prob\left(|\hat{\theta}_n-\theta^*|\geq\varepsilon;\theta^*\right)\leq\frac{\theta^*(1-\theta^*)}{n\varepsilon^2}$$
	We don't know $\theta^*$, but can deduce that $\theta^*(1-\theta^*)\leq\frac{1}{4}$ thus
	$$\prob\left(|\hat{\theta}_n-\theta^*|\geq\varepsilon;\theta^*\right)\leq\frac{1}{4n\varepsilon^2}$$
	To have a probability bound of our choosing we can substitute $\alpha=\frac{1}{4n\varepsilon^2}$ to get
	$$\prob\left(|\hat{\theta}_n-\theta^*|\geq\frac{1}{2\sqrt{n\alpha}};\theta^*\right)\leq\alpha$$
	Thus
	$$\prob\left(\hat{\theta}_n-\frac{1}{2\sqrt{n\alpha}}<\theta^*<\hat{\theta}_n+\frac{1}{2\sqrt{n\alpha}};\theta^*\right)\geq1-\alpha$$
	...
	\item We can improve on this crude bound.\\
	Let $W\sim\text{Normal}(0,1)$. We shall show that
	$$\frac{\sqrt{n}(\hat{\theta}_n-\theta^*)}{\sqrt{\hat{\theta}_n(1-\hat{\theta}_n)}}\to_\mathcal{D}W$$
	Note that $\hat{\theta}_n\to_\prob\theta^*$ by the \textit{Weak Law of Large Numbers}. In addition $\var(X_1)=\theta^*(1-\theta^*)$. The \textit{Central Limit Theorem} gives
	$$\frac{\sqrt{n}(\hat{\theta}_n-\theta^*)}{\sqrt{\hat{\theta}_n(1-\hat{\theta}_n)}}\to_\mathcal{D}W$$
	Now we define $Y_n=\dfrac{\sqrt{n}(\hat{\theta}_n-\theta^*}{\sqrt{\theta^*(1-\theta^*}}$ and $Z_n=\dfrac{\sqrt{\theta^*(1-\theta^*)}}{\sqrt{\hat{\theta}_n(1-\hat{\theta}_n}}$.\\
	The \textit{Continuous Mapping Theorem} tells us that $Z_n\to_{(\mathcal{D}\text{ or }\prob)}1$.\\
	Hence by \textit{Slutsky's Theorem} we obtain
...
%TODO relate this to when used
Let $\Theta$ be an open interval in $\reals$ and $f(x;\theta)$ be a pmf/pdf. Then they satisfy the following properties
\begin{enumerate}[label=\roman*)]
...
Let $\ell(\theta;x):=\ln f(x;\theta)$. The \textit{Score Function} defined on $\Theta\times\mathcal{X}$ is
$$\ell'(\theta;x):=\frac{d}{d\theta}\ell(\theta,;x)=\frac{\frac{d}{d\theta}f(x;\theta)}{f(x;\theta}$$
The score measures the sensitivity of the likelihood function to changes in $\theta$.

\remark{}
Note that under the \textit{Fisher Information Regularity Conitions} we have that $\forall\ theta\in|\Theta$
\[\begin{array}{rcl}
