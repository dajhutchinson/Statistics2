...
In this case $r=m-3=7-3=4$ since we have 7 classes of observations but 3 constraints due to the normal distribution.
```{r}
...
$$T_\text{pearson}(\X)=\sum_{j=1}^m\dfrac{(o_j-e_j)^2}{e_j}\to_\mathcal{D}\chi^2_r$$
where $o_j$ is the number of observations of class $j$ and $e_j$ is the expected number of observations of class $j$, assuming $H_0$ is true.\
In this case $r=`r length(obs)`-1=`r length(obs)-1`$ due to the breaks chosen in Question 1.
```{r}
mu<-70; sigma<-12
m<-length(obs) # number of classes

exp<-n*(pnorm(breaks[-1],mean=mu,sd=sigma)-pnorm(breaks[-length(breaks)],mean=mu,sd=sigma)) # expected number of outcomes
round(cbind(obs,exp),1) # display data table

t_obs<-sum((obs-exp)^2/exp) # observered test statistic
p_val<-1-pchisq(t_obs,df=m-1) # p-value
...
# Question 5
```{r}
set.seed(795430357) # Set RNG seed
...
simulate<-function(theta_hat_mle) {
   ...
   theta_hat_0.value<-maximise.ell(ell,score,X_rest,new_Y,rep(0,5))$value # Calculate MLEs
   theta_hat_mle.value<-maximise.ell(ell,score,X_full,new_Y,rep(0,9))$value
   
   t_obs<--2*(theta_hat_0.value-theta_hat_mle.value) # Calculate observed test statistic
}
...
$$T_\text{pearson}(\X)=\sum_{j=1}^m\dfrac{(o_j-e_j)^2}{e_j}\to_\mathcal{D}\chi^2_r$$
In this case $r=12-1=11$ since I split the observed data into twelve classes and there is one constraint on these classes.
```{r}
...
round(cbind(obs,exp),1) # Output data table

r<-length(obs)-1
t_obs<-sum((obs-exp)^2/exp) # observered test statistic
p_val<-1-pchisq(t_obs,df=r) # p-value
cat("df=",r,"\nt_obs=",t_obs,"\np_val=",p_val,sep="") # output test values
```
Here the $p$-value ($`r p_val`$) is not statistically significant enough to reject $H_0$, for a reasonable significance level.\
