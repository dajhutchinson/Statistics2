...
&=&\left\{\ell_n(\lambda;\x)>`r ell.mle`-2\right\}\\
&=&\left\{\ell_n(\lambda;\x)>`r ell.mle - 2`\right\}
\end{array}\]
...
Thus
\[\begin{array}{rcl}
C(\x)&=&\left\{\lambda\in\reals^{>0}:\ell_n(\lambda;\x)>\ell_n(\hat\lambda_{MLE};\x)-\frac{1}{2}`r quantile`\right\}\\
&=&\left\{\lambda\in\reals^{>0}:\ell_n(\lambda;\x)>`r ell.mle`-`r .5*quantile`\right\}\\
&=&\left\{\lambda\in\reals^{>0}:\ell_n(\lambda;\x)>`r ell.mle - .5*quantile`\right\}\\
&=&\{\lambda\in[`r min(z)`,`r max(z)`]\text{ from R}
\end{array}\]
We accept $H_0$ in this case.